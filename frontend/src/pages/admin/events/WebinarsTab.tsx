...
const WebinarsTab: React.FC<WebinarsTabProps> = ({ webinars: initialWebinars = [] }) => {
  ...
  const handleStatusChange = async (webinarId: number, newStatus: WebinarStatus) => {
    try {
  ...
  const handleToggleFeatured = async (webinarId: number) => {
    try {
      ...
      setWebinars(prev => 
        prev.map(w => w.id === webinarId ? {...w, isFeatured: !w.isFeatured} : w)
      );
    } catch (err) {
                      ...
                      {registrations.map((registration) => (
                        <tr key={registration.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div>
                              <div className="text-sm font-medium text-gray-900">{registration.attendeeName}</div>
