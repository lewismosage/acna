...
const WorkshopsTab: React.FC<WorkshopsTabProps> = () => {
  ...
  const [analyticsData, setAnalyticsData] = useState<WorkshopAnalytics | null>(
    null
  );
  ...
  const handleSaveWorkshop = async (workshopData: CreateWorkshopInput) => {
    try {
      if (editingWorkshop) {
        ...
        const updatedWorkshop = await workshopsApi.update(
          editingWorkshop.id,
          workshopData
        );
        setWorkshops((prev) =>
          prev.map((w) => (w.id === editingWorkshop.id ? updatedWorkshop : w))
        );
  ...
  const renderAnalyticsTab = () => {
      ...
      return (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            Analytics data will appear once you have workshops with
            registrations.
          </p>
        </div>
      );
    ...
    return (
      <div className="space-y-6">
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-blue-600 font-bold">
                    {analyticsData.workshopsByType.Online || 0}
                  </span>
                </div>
              </div>
              <div className="bg-green-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-green-600 font-bold">
                    {analyticsData.workshopsByType["In-Person"] || 0}
                  </span>
                </div>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-purple-600 font-bold">
                    {analyticsData.workshopsByType.Hybrid || 0}
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Top Workshops */}
        {analyticsData.topWorkshops &&
          analyticsData.topWorkshops.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  Top Workshops by Registration
                </h3>
                <button
                  onClick={async () => {
                    try {
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold text-green-600">
                        {workshop.registered} registrations
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-gray-900">
                  {analyticsData.total > 0
                    ? Math.round(
                        analyticsData.totalRegistrations / analyticsData.total
                      )
                    : 0}
                </span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-green-600">
                  {analyticsData.total > 0
                    ? Math.round(
                        (analyticsData.completed / analyticsData.total) * 100
                      )
                    : 0}
                  %
                </span>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-3">
              <button
                onClick={async () => {
                  try {
                    const blob = await workshopsApi.exportRegistrations(
                      undefined,
                      "csv"
                    );
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement("a");
                    a.href = url;
                    a.download = `workshop-registrations-${
                      new Date().toISOString().split("T")[0]
                    }.csv`;
                    document.body.appendChild(a);
                    a.click();
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Manage workshops, instructors, and educational sessions
              </p>
            </div>
            ...
            {[
              {
                ...
                count: workshops.filter((w) =>
                  ["Registration Open", "In Progress"].includes(w.status)
                ).length,
              },
              {
                id: "completed",
                label: "Completed",
                count: workshops.filter((w) => w.status === "Completed").length,
              },
              {
                id: "planning",
                label: "Planning",
                count: workshops.filter((w) => w.status === "Planning").length,
              },
              {
                id: "registrations",
                label: "Registrations",
            ...
            <div>
              <div className="mb-6">
                ...
                <p className="text-gray-600">
                  Comprehensive analytics and insights about your workshops
                </p>
              </div>
              {loading ? (
            ...
            <div className="space-y-6">
              {filteredWorkshops.map((workshop) => {
                ...
                const { truncated: truncatedDescription, hasMore } =
                  truncateToTwoSentences(workshop.description);
                          ...
                          <div className="relative">
                            <select
                              ...
                              onChange={(e) =>
                                handleStatusChange(
                                  workshop.id,
                                  e.target.value as WorkshopStatus
                                )
                              }
