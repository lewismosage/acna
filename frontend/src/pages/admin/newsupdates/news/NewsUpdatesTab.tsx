...
import { NewsItem, NewsStatus, NewsType } from "../types/types";
...
const NewsUpdatesTab: React.FC = () => {
  ...
  const fetchNews = async () => {
    try {
      ...
      setError(
        err.message || "Failed to load news items. Please try again later."
      );
    } finally {
      setLoading(false);
    }
  ...
  const toggleFeatured = async (id: number) => {
    try {
      ...
      setNewsItems(
        newsItems.map((item) => (item.id === id ? updatedNews : item))
      );
    } catch (error) {
  ...
  const getStatusIcon = (status: NewsStatus) => {
    switch (status) {
      ...
      case "Draft":
        return <DraftIcon className="w-3 h-3" />;
      case "Archived":
            ...
            <div className="space-y-4">
              <div className="bg-purple-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-purple-600 font-bold text-xl">
                    {analyticsData.monthlyViews.toLocaleString()}
                  </span>
                </div>
                <div className="mt-2">
              ...
              <div className="text-sm text-gray-600">
                <p>
                  ...
                  {analyticsData.published > 0
                    ? Math.round(
                        analyticsData.totalViews / analyticsData.published
                      )
                    : 0}
                </p>
                <p>
                  Featured article ratio:{" "}
                  {analyticsData.total > 0
                    ? Math.round(
                        (analyticsData.featured / analyticsData.total) * 100
                      )
                    : 0}
                  %
                </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600 mb-2">
                {analyticsData.total > 0
                  ? Math.round(
                      (analyticsData.published / analyticsData.total) * 100
                    )
                  : 0}
                %
              </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Manage news articles, press releases, and announcements
              </p>
            </div>
            ...
            <nav className="flex space-x-8">
              {[
                { id: "all", label: "All News", count: analyticsData.total },
                {
                  id: "published",
                  label: "Published",
                  count: analyticsData.published,
                },
                { id: "drafts", label: "Drafts", count: analyticsData.drafts },
                {
                  id: "archived",
                  label: "Archived",
                  count: analyticsData.archived,
                },
              ].map((tab) => (
                <button
                  key={tab.id}
                                ...
                                {item.author && (
                                  <div className="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div className="flex items-center gap-3">
                                      <img
                                        src={
                                          item.author.imageUrl || placeholderImg
                                        }
