...
const CreatePatientCareModal: React.FC<CreatePatientCareModalProps> = ({
  ...
  const resourceTypes: ResourceType[] = [
    'Guide', 'Video', 'Audio', 'Checklist', 'App', 'Website', 'Infographic', 'Handbook'
  ];
  ...
  const languages = [
    'English', 'French', 'Swahili', 'Hausa', 'Arabic', 'Portuguese', 'Amharic', 'Yoruba'
  ];

  const audienceOptions = [
    'Parents', 'Caregivers', 'Healthcare Providers', 'Teachers', 'Therapists', 'Family Members'
  ];

  const ageGroups = [
    '0-2 years', '2-5 years', '5-12 years', '12-18 years', '0-18 years', 'All Ages'
  ];
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      if (formData.type === 'Video' || formData.type === 'Audio') {
        if (!formData.duration.trim()) newErrors.duration = 'Duration is required for video/audio resources';
      }
      if ((formData.type === 'App' || formData.type === 'Website') && !formData.externalUrl.trim()) {
        newErrors.externalUrl = 'External URL is required for apps and websites';
      }
      if ((formData.type === 'Guide' || formData.type === 'Checklist' || formData.type === 'Handbook') && !formData.fileUrl.trim()) {
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">{formData.title.length}/150 characters</p>
      </div>
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">{formData.description.length}/300 characters</p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          maxLength={2000}
        />
        <p className="text-gray-500 text-sm mt-1">{formData.full_description.length}/2000 characters</p>
      </div>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-bold text-gray-900 mb-6">Audience & Metadata</h3>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {audienceOptions.map((audience) => (
            <label key={audience} className="flex items-center p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
              <input
                type="checkbox"
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      ...
      {(formData.type === 'Guide' || formData.type === 'Checklist' || formData.type === 'Handbook' || formData.type === 'Audio') && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            File URL {(formData.type === 'Guide' || formData.type === 'Checklist' || formData.type === 'Handbook') && '*'}
          </label>
      ...
      {(formData.type === 'App' || formData.type === 'Website' || formData.type === 'Video') && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            External URL {(formData.type === 'App' || formData.type === 'Website') && '*'}
          </label>
          <input
            type="url"
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-blue-700">{formData.targetAudience.join(', ')}</p>
          </div>
          <div>
      ...
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[99vh] overflow-hidden">
        ...
        <div className="bg-orange-50 px-6 py-4 border-b border-orange-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-orange-900 flex items-center">
            ...
            {editingResource ? 'Edit Patient Care Resource' : 'Create Patient Care Resource'}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
          ...
          <div className="text-center text-sm text-gray-600">
            Step {currentStep} of 4: {
              ...
              currentStep === 2 ? 'Audience & Metadata' :
              currentStep === 3 ? 'Files & Links' :
              'Review & Publish'
            }
          </div>
        </div>
