from rest_framework import serializers
from .models import EBooklet, EBookletView, EBookletDownload
import json
from django.core.files.storage import default_storage


class EBookletSerializer(serializers.ModelSerializer):
    # Computed fields for frontend compatibility
    image_url = serializers.SerializerMethodField()
    file_url = serializers.SerializerMethodField()
    
    class Meta:
        model = EBooklet
        fields = [
            'id', 'title', 'description', 'abstract', 'category', 'status', 
            'is_featured', 'image', 'image_url', 'file', 'file_url', 'file_size',
            'authors', 'target_audience', 'file_formats', 'table_of_contents',
            'keywords', 'tags', 'pages', 'language', 'isbn', 'publisher', 
            'edition', 'download_count', 'view_count', 'rating',
            'publication_date', 'created_at', 'updated_at', 'last_updated'
        ]
        read_only_fields = ['id', 'publication_date', 'created_at', 'updated_at', 'last_updated']
    
    def get_image_url(self, obj):
        """Get the image URL, prioritizing uploaded file over URL field"""
        if obj.image:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(obj.image.url)
            return obj.image.url
        return obj.image_url or '/api/placeholder/400/250'
    
    def get_file_url(self, obj):
        """Get the file URL, prioritizing uploaded file over URL field"""
        if obj.file:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(obj.file.url)
            return obj.file.url
        return obj.file_url or ''
    
    def validate_authors(self, value):
        """Ensure authors is a list of non-empty strings"""
        if isinstance(value, str):
            try:
                value = json.loads(value)
            except json.JSONDecodeError:
                value = [value] if value.strip() else []
        
        if not isinstance(value, list):
            raise serializers.ValidationError("Authors must be a list")
        
        # Filter out empty strings and ensure all items are strings
        return [str(author).strip() for author in value if str(author).strip()]
    
    def validate_target_audience(self, value):
        """Ensure target_audience is a list"""
        if isinstance(value, str):
            try:
                value = json.loads(value)
            except json.JSONDecodeError:
                value = [value] if value.strip() else []
        
        if not isinstance(value, list):
            raise serializers.ValidationError("Target audience must be a list")
        
        return [str(audience).strip() for audience in value if str(audience).strip()]
    
    def validate_file_formats(self, value):
        """Ensure file_formats is a list"""
        if isinstance(value, str):
            try:
                value = json.loads(value)
            except json.JSONDecodeError:
                value = [value] if value.strip() else []
        
        if not isinstance(value, list):
            raise serializers.ValidationError("File formats must be a list")
        
        return [str(format).strip() for format in value if str(format).strip()]
    
    def validate_table_of_contents(self, value):
        """Ensure table_of_contents is a list"""
        if isinstance(value, str):
            try:
                value = json.loads(value)
            except json.JSONDecodeError:
                value = [value] if value.strip() else []
        
        if not isinstance(value, list):
            raise serializers.ValidationError("Table of contents must be a list")
        
        return [str(item).strip() for item in value if str(item).strip()]
    
    def validate_keywords(self, value):
        """Ensure keywords is a list"""
        if isinstance(value, str):
            try:
                value = json.loads(value)
            except json.JSONDecodeError:
                value = [value] if value.strip() else []
        
        if not isinstance(value, list):
            raise serializers.ValidationError("Keywords must be a list")
        
        return [str(keyword).strip() for keyword in value if str(keyword).strip()]
    
    def validate_tags(self, value):
        """Ensure tags is a list"""
        if isinstance(value, str):
            try:
                value = json.loads(value)
            except json.JSONDecodeError:
                value = [value] if value.strip() else []
        
        if not isinstance(value, list):
            raise serializers.ValidationError("Tags must be a list")
        
        return [str(tag).strip() for tag in value if str(tag).strip()]
    
    def validate_pages(self, value):
        """Ensure pages is a positive integer"""
        if isinstance(value, str):
            try:
                value = int(value)
            except (ValueError, TypeError):
                raise serializers.ValidationError("Pages must be a number")
        
        if value < 0:
            raise serializers.ValidationError("Pages cannot be negative")
        
        return value
    
    def validate_isbn(self, value):
        """Basic ISBN validation"""
        if value and value.strip():
            # Remove hyphens and spaces for validation
            isbn_clean = value.replace('-', '').replace(' ', '')
            if not (len(isbn_clean) in [10, 13] and isbn_clean.replace('X', '').replace('x', '').isdigit()):
                raise serializers.ValidationError("Invalid ISBN format")
        return value
    
    def to_representation(self, instance):
        """Customize the output representation"""
        data = super().to_representation(instance)
        
        # Format dates for frontend consistency
        if data.get('publication_date'):
            data['publicationDate'] = data['publication_date']
        if data.get('created_at'):
            data['createdAt'] = data['created_at'].split('T')[0] if 'T' in str(data['created_at']) else str(data['created_at'])
        if data.get('updated_at'):
            data['updatedAt'] = data['updated_at'].split('T')[0] if 'T' in str(data['updated_at']) else str(data['updated_at'])
        
        # Rename fields for frontend compatibility
        data['downloadCount'] = data.get('download_count', 0)
        data['viewCount'] = data.get('view_count', 0)
        data['isFeatured'] = data.get('is_featured', False)
        data['targetAudience'] = data.get('target_audience', [])
        data['fileFormats'] = data.get('file_formats', [])
        data['tableOfContents'] = data.get('table_of_contents', [])
        data['fileSize'] = data.get('file_size', '0 MB')
        data['imageUrl'] = data.get('image_url', '/api/placeholder/400/250')
        data['fileUrl'] = data.get('file_url', '')
        data['publicationDate'] = data.get('publication_date', '')
        
        return data


class EBookletAnalyticsSerializer(serializers.Serializer):
    """Serializer for e-booklet analytics data"""
    total = serializers.IntegerField()
    draft = serializers.IntegerField()
    published = serializers.IntegerField()
    archived = serializers.IntegerField()
    featured = serializers.IntegerField()
    total_downloads = serializers.IntegerField()
    monthly_downloads = serializers.IntegerField()
    total_views = serializers.IntegerField()
    ebooklets_by_category = serializers.DictField(child=serializers.IntegerField())
    top_ebooklets = serializers.ListField(child=serializers.DictField())
    
    def to_representation(self, instance):
        """Convert snake_case to camelCase for frontend"""
        data = super().to_representation(instance)
        return {
            'total': data.get('total', 0),
            'draft': data.get('draft', 0),
            'published': data.get('published', 0),
            'archived': data.get('archived', 0),
            'featured': data.get('featured', 0),
            'totalDownloads': data.get('total_downloads', 0),
            'monthlyDownloads': data.get('monthly_downloads', 0),
            'totalViews': data.get('total_views', 0),
            'ebookletsByCategory': data.get('ebooklets_by_category', {}),
            'topEbooklets': data.get('top_ebooklets', []),
        }


class EBookletViewSerializer(serializers.ModelSerializer):
    class Meta:
        model = EBookletView
        fields = ['id', 'ebooklet', 'ip_address', 'user_agent', 'viewed_at']
        read_only_fields = ['id', 'viewed_at']


class EBookletDownloadSerializer(serializers.ModelSerializer):
    class Meta:
        model = EBookletDownload
        fields = ['id', 'ebooklet', 'ip_address', 'user_agent', 'downloaded_at']
        read_only_fields = ['id', 'downloaded_at']